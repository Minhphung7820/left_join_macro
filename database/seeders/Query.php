  ...
  protected function getBySell($business_id, $request)
  {
    ...
    if ($request['status'] != 'all' && $request['type'] == 'sell-return' && !isset($request['order_id'])) {
      ...
      if ($request['status'] != 'all' && $request['type'] == 'sell') {
        ...
        $query->leftJoinStockProductsCondition()
          ->leftJoin('transaction_sell_lines', function ($join) {
            ...
            // $join->leftJoin('transaction_sell_line_children', 'transaction_sell_line_children.parent_id', '=', 'transaction_sell_lines.id');
            // $join->on('transaction_sell_line_children.stock_id', '=', 'stock_products.stock_id');
          });

        $query->leftJoin(
          DB::raw('(SELECT * FROM transactions WHERE type = "price_quote" ORDER BY created_at DESC LIMIT 1) as transactions'),
          'transactions.id',
          '=',
          'transaction_sell_lines.transaction_id'
        );
        ...
        // $query->groupBy(['transaction_sell_lines.product_id', 'transaction_sell_lines.product_type', 'transaction_sell_lines.variation_id']);

        $query->leftJoin('crm_customers', 'crm_customers.id', '=', 'products.contact_id')
          ->where('products.business_id', $business_id);
        // lấy các sp đó đã được mua
        $query->leftJoin('transaction_sell_lines as lines_bought', function ($join) {
          $join->on('lines_bought.product_id', '=', 'products.id');
        })->leftJoin('transactions as transaction_bought', function ($join) {
          $join->on('transaction_bought.id', '=', 'lines_bought.transaction_id');
        })->where('transaction_bought.type', 'sell')
          ->where('transaction_bought.status', 'approve')
          ->where('transaction_bought.contact_id', $request['contact_id']);
        //
      ...
      $query
        ...
        ->leftJoin('stocks', function ($join) {
          $join->on('stocks.id', '=', 'stock_products.stock_id');
        })->leftJoin('crm_variant_attributes as tb_attr_1', 'tb_attr_1.id', '=', 'transaction_sell_line_children.attribute_first_id')
        ->leftJoin('units', 'units.id', '=', 'products.unit_id')
        ->where('transactions.contact_id', '=', $request['contact_id'])
        ...
        ->addSelect([
          ...
          'transaction_sell_line_children.stock_id',
          ...
          'transaction_sell_lines.product_id',
          'transaction_sell_line_children.stock_id',
          'transaction_sell_line_children.attribute_first_id'
        ]);
